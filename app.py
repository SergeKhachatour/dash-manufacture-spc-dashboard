...
def build_value_setter_line(line_num, label, value, col3):
    ...
    # For the input rows (USL, LSL, UCL, LCL)
    else:
        return html.Div(
            id=line_num,
            children=[
...
def build_value_setter_panel(dd_select, stored_data):
    ...
    panel_content = [
        ...
        build_value_setter_line('value-setter-panel-usl', 'Upper Specification limit',
                               str(state_dict[dd_select]['usl']), 'USL'),
        build_value_setter_line('value-setter-panel-lsl', 'Lower Specification limit',
                               str(state_dict[dd_select]['lsl']), 'LSL'),
        build_value_setter_line('value-setter-panel-ucl', 'Upper Control limit', 
                               str(state_dict[dd_select]['ucl']), 'UCL'),
        build_value_setter_line('value-setter-panel-lcl', 'Lower Control limit', 
                               str(state_dict[dd_select]['lcl']), 'LCL')
    ]

    # Return values for all outputs
    return (
        panel_content,  # for value-setter-panel children
        state_dict[dd_select]['usl'],  # for ud_usl_input value
        state_dict[dd_select]['lsl'],  # for ud_lsl_input value
        state_dict[dd_select]['ucl'],  # for ud_ucl_input value
        state_dict[dd_select]['lcl']   # for ud_lcl_input value
    )
...
def set_value_setter_store(set_btn, param, data, usl, lsl, ucl, lcl):
    ...
    if not any([usl, lsl, ucl, lcl]):  # If no new values entered
        return data
        
    # Update only the values that were entered
    if usl is not None:
        data[param]['usl'] = float(usl)
    if lsl is not None:
        data[param]['lsl'] = float(lsl)
    if ucl is not None:
        data[param]['ucl'] = float(ucl)
    if lcl is not None:
        data[param]['lcl'] = float(lcl)

    # Recalculate ooc in case of param updates
    data[param]['ooc'] = populate_ooc(df[param], data[param]['ucl'], data[param]['lcl'])
    ...
    if ctx.triggered:
        ...
        if prop_type == 'n_clicks':
            try:
                ...
                curr_id = params[1]  # Default to first parameter if there's an error
                
            prop_id = prop_id[:-7]  # Remove '_button' suffix
            if curr_id == prop_id:
                return generate_graph(interval, data, curr_id)
            else:
                return generate_graph(interval, data, prop_id)

        if prop_type == 'n_intervals':
            # Handle the case where cur_fig might be None
